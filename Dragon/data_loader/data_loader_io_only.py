...
def load_inference_data(_dict: DDict, 
    ...
    for i in range(file_chunk_num):
        ...
        print(f"Read iter {i}",flush=True)
        istart = i*num_files//file_chunk_num
        iend = min(istart + num_files//file_chunk_num, num_files)
        print(f"Reading files {istart} to {iend}", flush=True)
        imap_tic = perf_counter()
        smiles_sizes = pool.imap_unordered(
            read_smiles,
            file_tuples[istart:iend],
            chunksize=chunksize,
        )
        imap_toc = perf_counter()
        print(f"Imap launched in {imap_toc - imap_tic}s", flush=True)
        tic = perf_counter()
        for i, size in enumerate(smiles_sizes):
            toc = perf_counter()
            total_data_size += size
            print(f"Loaded file {istart + i} of size {size} bytes in {toc - tic}s, total size loaded: {total_data_size/(1024.*1024.*1024.)}", flush=True)
            tic = perf_counter()
       
        print(_dict.stats, flush=True)
        print("", flush=True)
